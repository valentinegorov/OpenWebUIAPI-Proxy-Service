...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Qwen Code natively enforces read-before-edit — skip the advisory (#1984, #2344, #2520).
    //
    ...
    //      Reliable across Qwen Code versions because it's schema, not env.
    //   2. `CLAUDE_CODE_ENTRYPOINT` / `CLAUDE_CODE_SSE_PORT` — env vars that
    //      Qwen Code does propagate to hook subprocesses (verified on
    //      Qwen Code CLI 2.1.116).
    //   3. `CLAUDE_SESSION_ID` / `CLAUDECODE` — kept for back-compat and in
    //      case future Qwen Code versions propagate them to hook
