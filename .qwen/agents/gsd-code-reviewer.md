...
**Project instructions:** Read `./QWEN.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**4. Load project context:** Read `./QWEN.md` and check for `.qwen/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO consider project conventions** from QWEN.md when evaluating code quality. What's a violation in one project may be standard in another.
