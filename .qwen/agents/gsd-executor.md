...
@/home/ve/work/OpenWebUIAPI-Proxy-Service/.qwen/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/home/ve/work/OpenWebUIAPI-Proxy-Service/.qwen/get-shit-done/references/ios-scaffold.md
...
@/home/ve/work/OpenWebUIAPI-Proxy-Service/.qwen/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @/home/ve/work/OpenWebUIAPI-Proxy-Service/.qwen/get-shit-done/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@/home/ve/work/OpenWebUIAPI-Proxy-Service/.qwen/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
When running inside a Qwen Code worktree (`.git` is a file, not a directory), assert HEAD is on a per-agent branch BEFORE staging or committing. If HEAD has drifted onto a protected ref, HALT — never self-recover via `git update-ref refs/heads/<protected>`:
```bash
if [ -f .git ]; then  # worktree
  ...
  # Positive allow-list: HEAD must be on the canonical Qwen Code worktree-agent
...
**Use template:** @/home/ve/work/OpenWebUIAPI-Proxy-Service/.qwen/get-shit-done/templates/summary.md
