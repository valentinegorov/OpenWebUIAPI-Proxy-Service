...
const CLAUDE_MD_FALLBACKS = {
  ...
  skills: 'No project skills found. Add skills to any of: `.qwen/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.',
};

// Directories where project skills may live (checked in order)
const SKILL_SEARCH_DIRS = ['.qwen/skills', '.agents/skills', '.cursor/skills', '.github/skills', '.codex/skills'];
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  try {
    ...
    // regardless of claude_md_path, so Codex projects never write to QWEN.md.
